...
**InsightVault AI: RAG-Based Knowledge Intelligence API | FastAPI, Python, RAG, Vector Search, Pydantic, Async Jobs, Pytest, GitHub Actions**

- Built a backend-first RAG knowledge intelligence API that ingests documents, chunks and indexes content, retrieves relevant context, and generates grounded answers with citations using an LLM provider abstraction and structured response validation.
- Implemented async ingestion jobs, citation guardrails, structured insight extraction, mock/OpenAI provider switching, evaluation datasets, and CI-tested backend workflows to demonstrate production-oriented LLM application engineering.
